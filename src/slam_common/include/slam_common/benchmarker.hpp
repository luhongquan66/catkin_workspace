...
#define BENCHMARK_START_NEW_STEP() slam_common::Benchmarker::notifyNewStepStart();
/// \brief Measure the time elapsed from this line to the end of the scope and record it to the
/// \c topic_name topic.
#define BENCHMARK_BLOCK(topic_name) slam_common::ScopedTimer __timer_ ## __LINE__(topic_name);
/// \brief Starts a measurement for the \c topic_name topic.
#define BENCHMARK_START(topic_name) slam_common::Benchmarker::startMeasurement(topic_name);
/// \brief Stops a measurement for the \c topic_name topic and store the result.
#define BENCHMARK_STOP(topic_name) slam_common::Benchmarker::stopMeasurement(topic_name);
/// \brief Stops a measurement for the \c topic_name topic and ignore the result.
#define BENCHMARK_STOP_AND_IGNORE(topic_name) \
  slam_common::Benchmarker::stopMeasurement(topic_name, true);
...
#define BENCHMARK_RECORD_VALUE(topic_name, value) \
    slam_common::Benchmarker::addValue(topic_name, static_cast<double>(value));
/// \brief Reset the topics with name starting with \c topic_prefix.
#define BENCHMARK_RESET(topic_prefix) slam_common::Benchmarker::resetTopic(topic_prefix);
/// \brief Reset all topics.
#define BENCHMARK_RESET_ALL() slam_common::Benchmarker::resetTopic("");
#else
